...
% find in-field peaks for each lap using Get_in_field_peak(), then average them
...
for p = 1:numel(MouseID_WT)
            ...
            if k > 1 && ~isnan(PF_loc(k-1)) && ~isnan(PF_loc(k))
                ...
                if ~isnan(BTSP_cells{Cell_ID,1})
                    ...
                    if BTSP_lap > 1
                        ...
                        peak_dFoF_before_BTSP(peak_dFoF_before_BTSP>=thr) = NaN;
                        a = [];
                        a = [st_history,nanmean(peak_dFoF_before_BTSP),];
    ...
    for j = 0:6
        ...
        n = stHis_residue(m,2);
        residue_stHis_WT(p,(j+1)) = nanmean(n);
    end
...
for p = 1:numel(MouseID_RTT)
    ...
    for j = 0:6
        m = find(stHis_residue(:,1) == j);
        n = stHis_residue(m,2);
        residue_stHis_RTT(p,(j+1)) = nanmean(n);
    end

end

%%
WT = residue_stHis_WT(:,2:7);
RTT = residue_stHis_RTT(:,2:7);
...
%print(gcf, 'fig4e.pdf','-dpdf','-vector','-bestfit');
